				 ...
				 :style="'height:'+windowHeight+'px'">
					...
					<view class="content3 animated fadeInLeft scrollheight" v-if="recommendProducts.length>0 && showResult === 1">
						...
						<view class="itemWrapper" v-for="(value,index) in recommendProducts" :key="index">
							...
							<view class="advisory" v-if="value.infos.length>0">
								<view v-for="(item,m) in value.infos" :key="m">
									...
									<view v-if="item.questionShow === false" class="arrow" @click="questionShowOrNot(true,index,m)">
										查看更多
										<!-- <image src="../../static/images/circle-arrow-down.png"></image> -->
									</view>
									<view v-if="item.questionShow === true" class="arrow" @click="questionShowOrNot(false,index,m)">
										收起
						...
						<view class="footer">
							...
							<button class="kefu" :session-from="'nickName='+userInfo.nick+'|'+'country='+userInfo.country+'|'+'city='+userInfo.city+'|'+'avatarUrl='+userInfo.ico+'|'+'gender='+userInfo.sex+'|'+'province='+userInfo.province" open-type="contact"></button>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		onShow() {
			...
			this.QUES_ILLS = getApp().globalData.QUES_ILLS;
