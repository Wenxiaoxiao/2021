...
let httpList={
     ...
     */
    //微信
    'wxminiSendMessage': {url: '/uc/template/api/wxmini'},
    //支付宝
    'aliminiSendMessage': {url: '/uc/template/api/alimini'},
    //获取当前订单的成员分类信息
    p_queryInsureders:{"url":"/policy/wtPolicys/api/queryInsureders"},
    //获取小程序用户信息
    p_WX_userInfo:{"url":"/uc/login/thirdOauthBaseInfo"},

    //绑定小程序手机号
    p_xcx_BDMobile:{"url":"/uc/login/phoneRegister"},
  //获取成员分类列表
  P_bind_newRelations:{url:'/uc/ucUser/api/relations',type:'post'},

  //获取保单统计数据
  P_policyCount:{url:'/policy/wtPolicys/api/policyCount',type:'post'},

  //获取保单列表
  P_policyList:{url:'/policy/V4_4/api/list',type:'post'},

  // 保险种类保单统计
  P_classifyPolicyCount:{url:'/policy/wtPolicys/api/classifyPolicyCount',type:'post'},

  // 查询被邀请用户
  P_policyQuery:{url:'/policy/wtPolicys/api/queryShareUser',type:'post'},

  // 獲取我的家庭成員
  P_policyIsSared:{url:'/policy/wtPolicys/api/iShared',type:'post'},

  //获取手机验证码
  getPhoneCode:{type:'post'},

  //绑定手机号
  BDMobile:{url:"/uc/login/phoneRegister",type:'post'},

  //快速理赔
  P_claimsDetail:{url:"/policy/policy/api/policyDetailByPolicyNo",type:'post'},

  //提交理赔申请
  P_addClaim:{url:"/policy/claimSettle/api/addClaim",type:'post'},
 ...
 //保单邀请
    ...
    // 评论
    Exe_agentComments: {
        url: 'https://bx.wts999.com/wt/policyAgentComments/api/comment',
        type: 'post'
    },
    // 单独传入url
    none: {
